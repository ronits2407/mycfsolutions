...
 
   ...
   set<int> middle_permutation;
   for (int i = sideways; i < n - sideways; i++)
   {
        middle_permutation.insert(a[i]);
   }
 
   for (int i = sideways; i < n - sideways; i++)
   {
        if (b[i] != -1 && !middle_permutation.count(b[i]))
        {
            cout << "NO" << endl;
            return;
        }
