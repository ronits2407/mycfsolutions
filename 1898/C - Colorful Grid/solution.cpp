...
";
    ...
    cout << "YES
";
    vector<vector<char>> grid(n, vector<char>(m - 1, 'R'));
    for(int i = 0; i < m - 1; i++)
    {
        grid[0][i] = (i % 2 == 0) ? 'R' : 'B';
    }
    ...
    for(int i = 0; i < n - 1; i++)
    {
        grid2[i][m - 1] = (i % 2 == temp) ? 'R' : 'B';
    }
    
    
 
    grid[n - 1][m - 2] =((grid2[n - 2][m - 1] == 'B') ? 'B':'R');
    grid[n - 2][m - 2] = grid[n - 1][m - 2];
    grid2[n - 2][m - 2] = ((grid2[n - 2][m - 1] == 'R') ? 'B':'R');
 
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m - 1; j++)
