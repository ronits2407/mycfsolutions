...
 
void build(int curr, int parent, int x, int y, vector<vector<int>>& edges, bool& possible)
{
    int nodesramain = x + y;
    if (!nodesramain)
    {
...
 
    if (nodesramain % 2 == 0)
    {
        if (x > 0)
        {
            if (parent != -1)
            {
                edges[parent].push_back(curr);
            }
            
            build(curr + 1, curr, x - 1, y, edges, possible);
        }
        else
        {
            if (parent == -1)
            ...
            {
                for (int j = curr; j < edges.size(); j++)
                {
                    edges[parent].push_back(j);
                }
    ...
    else
    {
        if (y > 0)
        {
            ...
            build(curr + 1, curr, x, y - 1, edges, possible);
        }
        else
        {
            possible = false;
...
 
    ...
    cin >> x >> y;
 
    vector<vector<int>> edges(x + y + 1);
 
    bool possible = true;
    build(1, -1, x, y, edges, possible);
    if (possible)
    {
        cout << "YES" << endl;
        for (int i = 1; i <= x + y; i++)
