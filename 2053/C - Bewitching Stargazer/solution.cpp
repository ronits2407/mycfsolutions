...
 
 
pii calc(int n, int k, int l, int r)
{
    int len = r - l + 1;
    int mid = (l + r)/2;
    if (len < k)
...
 
    if (len % 2 == 0)
    {
        auto [lucky, count] = calc(n, k, l, mid);
        return {2 * lucky + count * (len/2), 2 * count};
 
    }
    else
    {
        auto [lucky, count] = calc(n, k, l, mid -1);
        return {2 * lucky + count * ((len  + 1)/2) + mid, 2 * count + 1}; 
    }
