...
 
    vector<int> smin(n, INF);
    for (int i = n - 2; i >= 0; i--)
    {
        smin[i] = min((int)a[i + 1], smin[i + 1]);
    }
 
    int basemoves = 0;
    for (int i = 0; i < n; i++)
...
 
    if (idxpossible.size() > 0)
    {
        found_any = true;
        maxextra = idxpossible[0];
 
        for (int i = 1; i < idxpossible.size(); i++)
        {
            int currdist = idxpossible[i] - idxpossible[i - 1] - 1;
            if (currdist > maxextra)
            {
                maxextra = currdist;
            }
        }
