...
 
        int minimum_first = *first.begin();
        int to_added_new;
        
            to_added_new = second.front();
        
        
 
        if (minimum_first != -1 && m - minimum_first < 0)
        {
            break;
        }
        else if (minimum_first != -1 && m - minimum_first >= 0)
        {
            m -= minimum_first;
            first.erase(first.begin());
            first.insert(to_added_new);
            
                second.pop();
            
            
            second.push(minimum_first);
        }
        else if (minimum_first == -1 && m - winweight < 0)
        {
            break;
        }
        else
