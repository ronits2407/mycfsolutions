...
 
void dfs(map<int, set<int>>& tree, int m, int mcount, int& answer, vector<int>& visited, int currnode, vector<int>& cats)
{
    bool leaf = true;
    visited[currnode] = 1;
...
 
    ...
    debug(currnode, mcount);
    
    for(auto& neigh: tree[currnode])
    {
        debug(neigh);
        if(!visited[neigh] && mcount <= m)
        {
            leaf = false;
            dfs(tree, m, mcount, answer, visited, neigh, cats);
        }
    }
...
 
    ...
    int answer = 0;
 
    dfs(tree, m, 0, answer, visisted, 1, a);
    cout << answer << endl;
    return 0;
}