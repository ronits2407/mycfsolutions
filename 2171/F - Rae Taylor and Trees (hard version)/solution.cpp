...
 
    stack<int> CCs;
    map<int, set<int>> adj;
    CCs.push(permutation[0]);
    for (int i = 1; i < n; i++)
    {
        if (permutation[i] < CCs.top())
        {
            CCs.push(permutation[i]);
            continue;
        }
        else
        {
            int curr_min = permutation[i];
            while (!CCs.empty() && permutation[i] > CCs.top())
            {
                int a = CCs.top();
                int b = permutation[i];
                // we have b > a
