...
 
    ...
    ask(b, n);
 
    vector<pair<int, int>> ajd_gcds(n);
    int count = 0;
    ajd_gcds[0].first = gcd(a[0], a[1]);
    ajd_gcds[0].second= gcd(a[0], a[1]);
    ajd_gcds[n-1].first = gcd(a[n-1], a[n-2]);
    ajd_gcds[n-1].second = gcd(a[n-1], a[n-2]);
 
    if(ajd_gcds[0].first < a[0])
    {
        count++;
    }
 
    if(ajd_gcds[n-1].first < a[n-1])
    {
        count++;
    }
    for (int i = 1; i < n-1; i++)
    {
        ajd_gcds[i].first = gcd(a[i], a[i-1]);
        ajd_gcds[i].second = gcd(a[i], a[i+1]);
 
        int lcm = ajd_gcds[i].first * ajd_gcds[i].second / gcd(ajd_gcds[i].first , ajd_gcds[i].second);
 
        if(lcm < a[i])
        {
            count++;
