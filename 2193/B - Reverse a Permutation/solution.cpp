...
 
    sort(a.rbegin(), a.rend()); // reverse
 
    // first index where a and b differ
    int index = -1;
    for (int i = 0; i < n; i++) {
        if (a[i] != b[i]) {
...
 
    // find the index of greatest element in b after index , including the indexx
    if (index == -1) {
        give(b, n);
        return;
    }
 
    int max_index = max_element(b.begin() + index - 1, b.end()) - b.begin();
 
    // reverse from index to max_index in b
    reverse(b.begin() + index - 1, b.begin() + max_index + 1);
    give(b, n);
