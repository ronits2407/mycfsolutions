...
 
            ...
            int k = tree.size();
            int height = (int)floor(log2(k));
            if ((k & (k + 1)) == 0)
            {
                int temp2 = (int)pow(2, height);
                while (b > 0)
                {
                    // debug(b, height);
                    b -= temp2;
            ...
            remainingcount /= 2;
            debug(prevremainingcount, remainingcount, tc);
            b -= remainingcount;
            while (b > 0)
            {
                debug(b, height);   
                b -= (remainingcount + prevremainingcount);
                height++;
            }
            cout << height << endl;
