...
 
    ...
    {
        int required = a[i];
        auto itterright = upper_bound(prefix.begin() + i, prefix.end(), required + prefix[i - 1]);
        int idx = itterright - prefix.begin();
        ...
        if(idx <= n)
        {
            remaining[idx] += required - (prefix[idx-1] - prefix[i - 1]);
        }
...
 
    ...
    for (int i = 1; i <= n; i++)
    {
        cout << remaining[i] + (b[i] * complete[i]) << " ";
    }
