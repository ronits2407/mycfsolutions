...
 
        for (int j = 0; j < update.size(); j++)
        {
            ...
            else
            {
                if (curr == 'I')
                {
                    int remainingsingletables = x - j;
                    if(remainingsingletables > 0)
                    {
                        update[j + 1] = max(update[j + 1], occupied[j] + 1);
                    }
                }
                else if (curr == 'A')
                {
                    ...
                    if (j > 0 && j * s - occupied[j] > 0)
                    {
                        update[j] = max(update[j], occupied[j] + 1); 
                    }
