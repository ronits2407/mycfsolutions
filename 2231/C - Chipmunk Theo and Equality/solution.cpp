...
 
    ask(a, n);
 
    unordered_map<int , pii> totalvisitcount_sumstepcount;
    for (int i = 0; i < n; i++)
    {
        set<int> nums;
        int curr = a[i];
        ...
        {
            nums.insert(curr);
            totalvisitcount_sumstepcount[curr].first++;
            totalvisitcount_sumstepcount[curr].second+= count++;
        ...
        {
 
            totalvisitcount_sumstepcount[2].first++;
 
            totalvisitcount_sumstepcount[2].second += count++;
            
            totalvisitcount_sumstepcount[1].first++;
 
            totalvisitcount_sumstepcount[1].second += count;
        } 
        else if (curr == 1) 
        {
 
...
 
    ...
    int min_ = INF;
    for (auto &&entryr : totalvisitcount_sumstepcount)
    {
        if (entryr.second.first == n)
        {
            min_ = min(min_, entryr.second.second);
        }
