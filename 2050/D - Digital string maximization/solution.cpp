...
 
    ...
    {
        pair<int, char> bestoption = {i, s[i]};
        for (int j = i + 1; j < min<int>(i + 10, s.size()); j++)
        {
            int num = s[j] - '0';
            if (num - (j - i) > bestoption.second - '0')
            {
                bestoption = {j, s[j] - (j - i)};
            }
            
        }
 
        ans += bestoption.second;
        for (int j = bestoption.first; j > i; j--)
        {
            s[j] = s[j - 1];
        }
