...
 
    ...
    {
        if (nearest_distance == rhs.nearest_distance)
        {
        ...
        return nearest_distance < rhs.nearest_distance;
    }
 
    friend ostream& operator<<(ostream& out, const Node& node)
    {
        out << "Nearest Distance " << node.nearest_distance << " position " << node.position << " direction: " << node.direction << endl;
        return out;
    }
};
 
...
 
    ...
    set<Node> options;
 
    options.insert({a[0], 0, 'R'});
    options.insert({x - a[n - 1] ,x, 'L'});
    for (int i = 0; i < n - 1; i++)
    {
        options.insert(Node{(a[i + 1] - a[i]) / 2, (a[i + 1] + a[i]) / 2, 'L'});
        options.insert(Node{(a[i + 1] - a[i] - 1) / 2, ((a[i + 1] + a[i]) / 2) + 1, 'R'});
    }
    for (auto &&i : options)
    {
        debug(i);
...
 
    ...
    while (k--)
    {
        Node best = *options.rbegin();
        options.erase(best);
 
        Node next = best;
        bool added = positions.insert(best.position).second;
 
        if (!added)
        {
            k++;
...
 
 
        if (next.nearest_distance > 0)
        {
            if (next.direction == 'R')
            {
                next.nearest_distance--;
                next.position++;
            }
            else
            {
