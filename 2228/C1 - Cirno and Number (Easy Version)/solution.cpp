...
 
 
void check(int a, int d1, int d2,string& given, int& temp,int i, bool less, bool greater, int curr, int& min_)
{
    if(i == temp)
    {
...
 
    if (less)
    {
        check(a, d1, d2, given, temp, i + 1, true, false, curr * 10 + d2, min_);
    }
    else if(greater)
    {
        check(a, d1, d2 ,given , temp, i + 1, false, true, curr * 10 + d1, min_);
    }
    else
    {
        check(a, d1, d2 ,given , temp, i + 1, d1 < digit, d1 > digit, curr * 10 + d1, min_);
        check(a, d1, d2 ,given , temp, i + 1, d2 < digit, d2 > digit, curr * 10 + d2, min_);
    }
...
 
     int temp = digitcount;
     int option1 = (d1 == 0) ? d2 : d1;
     for (int i = 0; i < temp; i++) {
         option1 = option1 * 10 + d1;
     }
 
     int option2 = 0;
     for (int i = 0; i < temp - 1; i++) {
         option2 = option2 * 10 + d2;
     }
 
    //  debug(option1);
 
     ...
     {
        check(a, d1, d2 ,given , temp, 0, false, false, 0, min_);
        cout << min_ << endl;
        return;
     }
